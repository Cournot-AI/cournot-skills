...
test("development is the default and explicit production requests stay on production", async (t) => {
  const { credentials, intents } = fixture(t);
  assert.equal(PRODUCTION_BASE, "https://interface.cournot.ai");
  assert.throws(() => apiBase("https://pro.cournot.ai"), /Unsupported/);
  for (const base of [undefined, PRODUCTION_BASE]) {
    await prepareProbability({ request, credentials, intents, ...(base ? { base } : {}),
      ...
      fetchImpl: async (url) => {
        assert.equal(url, `${base || DEV_BASE}/intelligence/v1/probability`);
